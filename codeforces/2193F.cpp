...
void _GO() {
  ...
  for (int i = 0; i < n; ) {
      ...
      int_64 dist = mx - mn;
      int_64 nl = min(l + abs(yl - mx), h + abs(yh - mx)) + dist;
      int_64 nh = min(l + abs(yl - mn), h + abs(yh - mn)) + dist;
  ...
  int_64 ans = min(l + abs(yl - by), h + abs(yh - by)) + (bx - ax);
  cout << ans % 1000000007 << '\n';
}
