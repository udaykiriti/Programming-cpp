...
void _GO() {
  ...
  vector<char> cvrd(unq, 0);

  unordered_map<int_64,int> cnt;
  cnt.reserve(unq * 4);

  for (int_64 x : pre) {
    int_64 val = x;

    vector<pair<int_64,int>> fac;
    for (int p : prs) {
      if ((int64_t)p * p > val) 
        break;
      if (val % p == 0) {
    ...
    if (val > 1) fac.eb(val, 1);
    vector<int_64> divs = {1};
    for (auto &pr : fac) {
      int_64 p = pr.F; 
      int e = pr.S;
      int sz = divs.size();
  ...
  for (int_64 b : ok) {
    bool take = false;
    for (int_64 m = b; m <= mxa; m += b) {
      auto it = ind.find(m);
      if (it != ind.end() and !cvrd[it->S]) { take = true; break; }
    }
    if (!take) continue;
    ans.push_back(b);
    for (int_64 m= b; m<= mxa; m += b) {
