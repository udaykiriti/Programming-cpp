...
int_64 ok(int_64 x, int_64 y, int_64 limit) {
    if (x == 0 || y == 0) 
        return 0;
    int_64 g = gcd(x, y);
    int_64 div = x / g;
...
void _GO() {
  ...
  int_64 na = m / a,nb = m/ b, nc = m/c;

  int_64 lab = ok(a, b, m);
  int_64 lbc = ok(b, c, m);
  int_64 lac = ok(a, c, m);
  int_64 labc = ok(lab, c, m);
  ...
  int_64 da = na -nab - nac + nabc;
  int_64 db = nb -nab - nbc + nabc;
  int_64 dc = nc - nac - nbc + nabc;

  int_64 wa = da * 6 + dab * 3 + dac * 3 + dabc * 2;
  int_64 wb = db * 6 + dab * 3 + dbc * 3 + dabc * 2;
  int_64 wc = dc * 6 + dac * 3 + dbc * 3 + dabc * 2;
