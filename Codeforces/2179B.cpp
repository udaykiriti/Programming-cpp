...
void _GO() {
  ...
  int_64 t0T(0);
  for (int i = 0; i + 1 < n; i++) {
    t0T += abs(vec[i] - vec[i + 1]);
  }
  int_64 ans = t0T;
  ans = min(ans,t0T- abs(vec[0]- vec[1]));
  ans = min(ans,t0T - abs(vec[n- 2]- vec[n- 1]));
  for (int i = 1; i + 1 < n; i++) {
    int_64 cur = t0T - abs(vec[i - 1] - vec[i]) - abs(vec[i] - vec[i + 1]) + abs(vec[i - 1] - vec[i + 1]);
    ans = min(ans, cur);
  }
  cout << ans << '\n';
}
