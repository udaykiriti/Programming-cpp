...
void _GO() {
    ...
    FOR(i,0,n) 
    {
        (vec[i] % 2 == 0) ? evns.push_back(vec[i]) : oDs.push_back(vec[i]);
    }

    sort(evns.begin(), evns.end(), greater<int_64>());
    sort(oDs.begin(), oDs.end(), greater<int_64>());

    int evnsz = (int)evns.size(), oddsz = (int)oDs.size();

    vl pr(evnsz + 1, 0);
    for (int i = 0; i < evnsz; ++i) pr[i+1] = pr[i] + evns[i];
    if (0 == oddsz) 
    {
        for (int k = 1; k <= n; ++k) 
        {
    ...
    for (int k = 1; k <= n; ++k) 
    {
        ...
        for (int m = szz; m >= 0; --m) 
        {
            ...
            int Weak = max(0, r - reme),strong = min(remo, r);
            if (Weak > strong)
            {
                continue;
            }

            int x = (Weak % 2 == 0) ? Weak : (Weak + 1);
            if (x <= strong) 
            {
                top = oDbst + pr[m];
                ok = true;
