...
// #define ONPC
const int64_t TMP = -2e18;

int64_t fget(vector<int64_t>& vec, int cnt) {
    if (0 == cnt || vec.empty()) return 0;
    
    int64_t tp{0};
    sort(vec.rbegin(), vec.rend());
    int lmt = min((int)vec.size(), cnt);
    // for (int i{0}; i < cnt; i++) {
    //     int bst{-1};
        
    //     for (int j{0}; j < vec.size(); j++) {
    //         if (0 == i || vec[j] > 0) { 
    //             if (-1 == bst || vec[j] > vec[bst]) 
    //                 bst = j;
    //         }
    //     }
    if(lmt > 0){
...
void _GO() {
  ...
  for(int i = 1; i <= n; i++){
    ...
    cin >> x;
    t0T += x;
    if(i & 1) ods.push_back(x);
    else eves.push_back(x);
  }
  int Ocnt{0},Ecnt{0};
  for(int i = 0; i < m; i++){
    int x;
  ...
  int64_t sum = fget(ods, Ocnt) + fget(eves, Ecnt);
  cout << t0T - sum << endl;
}
