...
int f(int u, int v) {
    ...
    int_64 res = 1, t = a[v] - a[u];
    
    if (t > 0) {
        for (int w : rg[u]) 
            if (a[w] == t && w != v) res = (res + f(w, u)) % M;
    }
    return dp[{u, v}] = res;
}
 ...
 * Fix it
 * Prev sol timelimit exceeded 
 * Now on tc 11 same
 * Need to find some more optimal
 * extend this or do diff thng...
 */
