...
int_64 solve(int_64 P, int_64 C, int_64 V) {
    return max(V + C, P - V);
}
...
void _GO() {
  ...
  int_64 n, x, y;
    ...
    if (c1 == 0) {
        if (solve(p0, c0, y) <= x) cout << "YES" << '\n';
        else cout << "NO" << '\n';
    } else if (c0 == 0) {
        if (solve(p1, c1, x) <= y) cout << "YES" << '\n';
        else cout << "NO" << '\n';
    } else {
        int_64 tmp1 = max(0LL, (p0 - c0) / 2);
        int_64 tmp2 = max(0LL, (p1 - c1) / 2);
        if (tmp2 + solve(p0, c0, tmp1) <= x && tmp1 + solve(p1, c1, tmp2) <= y) {
            cout << "YES" << '\n';
        } else {
            cout << "NO" << '\n';
        }
