...
bool ok(int_64 x, int n, int m, int l, const vi& a) {
    if (x >= l) 
        return true;
    ...
    FOR ( i , 0 , n) {
        ...
        curr += (a[i] - lst);
        curr -= min((int_64)x , curr);
        if (curr > (int_64)(m - 1) * x) 
            return false;
...
void _GO() {
  ...
  int_64 low = 0, high = l, ans = l;

  while (low <= high) {
    int_64 mid = low + (high - low) / 2;
    if (ok(mid, n, m, l, a)) {
        ans = mid;
        high = mid - 1;
    } else {
