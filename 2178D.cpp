...
void _GO() {
  ...
  int_64 n,m;
  cin >> n >> m;
  vector<pair<int_64,int_64>> vec(n);
  for(int_64 i{0} ; i < n ; i++){
    cin >> vec[i].F,vec[i].S = i+1;
  }
  SORT(vec);
  if(m == 0) {
    if(2 == n) {
    ...
    for(int i{0}; i < hook; i++) {
       cout << vec[i].S << " " << vec[n-1].S << '\n';
    }
    for(int i{hook+1}; i < n; i++) {
      cout << vec[i-1].S << " " << vec[i].S << '\n';
    }
    return;
   }
   else if(1 == m) {
   ...
   for(int_64 i{t1}; i < t2; i++){
    cout << vec[i].S << " " << vec[i+m].S << '\n';
   }
   for(int_64 i{n-1}; i>= t3; i--){
    cout << vec[i].S << " " << vec[i-m].S <<'\n';
   }
}
