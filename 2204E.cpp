...
void _GO() {
  ...
  int_64 start= max(1LL, t0T - 100);
    
  for (int_64 val= start; val <= t0T; ++val) {

    vector<str> arr;

    int_64 curr =val ,runsum = val;

    arr.pb(to_string(curr));
        
    while (curr > 9) {
        int_64 nxt = 0, temp = curr;
